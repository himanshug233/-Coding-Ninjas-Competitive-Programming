...
int longestBitonicSubarray(int *input, int n) {
    int *longest_increasing_subsequence_at_curr_position = lis(input, n);
    int *longest_decreasing_subsequemce_at_curr_position = lds(input, n);
    
    int best = 2;
    
    for(int i = 0; i<n; i++){
        best = max(best, longest_increasing_subsequence_at_curr_position[i] + longest_decreasing_subsequemce_at_curr_position[i] - 1);
    }
    return best;
}
