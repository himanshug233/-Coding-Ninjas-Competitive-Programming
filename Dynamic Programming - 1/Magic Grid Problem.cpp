...
int health(int** input, int r, int c, int** temp){
    temp[r-1][c-1] = 1;
    temp[r-1][c-2] = 1;
    temp[r-2][c-1] = 1;
    
    for(int i = r-3; i>=0; i--){
        temp[i][c-1] = temp[i+1][c-1] - input[i+1][c-1];
        if(temp[i][c-1] <= 0){
            temp[i][c-1] = 1;
        }
    }
    ...
    for(int j = c-3; j>=0; j--){
        temp[r-1][j] = temp[r-1][j+1] - input[r-1][j+1];
        if(temp[r-1][j] <= 0){
            temp[r-1][j] = 1;
        }
    }
    ...
    for(int i = r-2; i>=0; i--){
        for(int j = c-2; j>=0; j--){
            ...
            down = temp[i+1][j] - input[i+1][j];
            if(down <= 0){
            down = 1;
            }
            
            right = temp[i][j+1] - input[i][j+1];
            if(right <= 0){
            right = 1;
            }
